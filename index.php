...
// จัดการการลบข้อมูล (Delete) พร้อมลบไฟล์รูปภาพออกจากโฟลเดอร์เพื่อประหยัดพื้นที่
if (isset($_GET['delete_id'])) {
    // ดึงชื่อไฟล์รูปภาพเก่าออกมาก่อนลบ
    $stmtImg = $pdo->prepare("SELECT image FROM foods WHERE id = ?");
    $stmtImg->execute([$_GET['delete_id']]);
    $oldImg = $stmtImg->fetchColumn();

    // ลบไฟล์รูปในโฟลเดอร์ uploads (ถ้ามี)
                                ...
                                <tr>
                                    <td>
                                        <?php if (!empty($food['image']) && file_exists("uploads/" . $food['image'])): ?>
                                            <img src="uploads/<?= htmlspecialchars($food['image']) ?>" alt="food"
                                                class="img-thumbnail" style="width: 80px; height: 60px; object-fit: cover;">
                                        <?php else: ?>
                                            <div class="bg-secondary text-white rounded text-center d-flex align-items-center justify-content-center small"
                                                style="width: 80px; height: 60px;">No Image</div>
                                        <?php endif; ?>
                                    </td>
                                    ...
                                    <td class="text-center">
                                        ...
                                        <a href="index.php?delete_id=<?= $food['id'] ?>" class="btn btn-sm btn-danger"
                                            onclick="return confirm('คุณแน่ใจหรือไม่ที่จะลบเมนูนี้? ข้อมูลวัตถุดิบทั้งหมดจะถูกลบไปด้วย');">ลบ</a>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        <?php endif; ?>
